...
function get_m4a(message_id) {
  const url = `https://api-data.line.me/v2/bot/message/${message_id}/content`;
  var options = {
    method: "GET",
    headers: { Authorization: `Bearer ${this.ACCESS_TOKEN}` },
  };
  return UrlFetchApp.fetch(url, options).getBlob().setName(`${message_id}.m4a`);
}

// Function to send a Flex Message reply
function reply_send_flex(reply_token, flex_message) {
  let url = "https://api.line.me/v2/bot/message/reply";
  const res = UrlFetchApp.fetch(url, {
    headers: {
      "Content-Type": "application/json; charset=UTF-8",
...
function pushMessage_send_flex(to, flex_message) {
  let url = "https://api.line.me/v2/bot/message/push";
  let headers = {
    "Content-Type": "application/json; charset=UTF-8",
    Authorization: "Bearer " + this.ACCESS_TOKEN,
...
function broadcastMessage_send_flex(flex_message) {
  let url = "https://api.line.me/v2/bot/message/broadcast";
  let headers = {
    "Content-Type": "application/json; charset=UTF-8",
    Authorization: "Bearer " + this.ACCESS_TOKEN,
